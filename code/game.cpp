...
function void
hv_append(SpriteBatch *sb, Vec4 sprpos, Rect atlpos, Vec3 color)
{
    sb->sprite[sb->count].pos = 	{ sprpos.x, sprpos.y };
    sb->sprite[sb->count].size = 	{ sprpos.z, sprpos.w };
    sb->sprite[sb->count].aoffset = {atlpos.x, atlpos.y};
    sb->sprite[sb->count].asize = 	{atlpos.w, atlpos.h};
    sb->sprite[sb->count].color = 	color;
    sb->count++;
}
...
{
	Rect trect = atlas_textures[tname].rect;
...
{
	Rect sprite = atlas_textures[tname].rect;
	Vec4 sb_pos = {pos.x, pos.y, sprite.w, sprite.h};
...
UPDATE_FUNC(game_update)
{
	...
	draw_rectangle(m->sb, {50, 500, 200, 20});
